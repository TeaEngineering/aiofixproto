...
from . import message, FIXValidator
import re
import unittest
...
class TestValidators(unittest.TestCase):
    def test_data_types(self):
        testField = FIXValidator.StringField(324)
        testField.known_as('test')
        data_dict = {}
        testField.parse(b'HI', data_dict, None)

        testField = FIXValidator.IntField(345)
    ...
    def test_msg_validate(self):
        msg = message.from_delim('8=FIX.4.2|9=49|35=A|34=1|49=ARCA|52=20150916-04:14:05.306|56=TW|10=169|')
        ...
        v = FIXValidator.BaseFIXValidator()
        with self.assertRaisesRegex(FIXValidator.RejectError, 'Unsupported MsgType A') as cm:
            data = v.validate(msg)
        self.assertEqual(cm.exception.sessionRejectReason, 11)

        m = FIXValidator.Message('A', 'Logon')
        ...
        with self.assertRaisesRegex(FIXValidator.RejectError, 'Unexpected tag 553 found in Logon message') as cm:
            data = v.validate(msg)
        self.assertEqual(cm.exception.sessionRejectReason, 2)

        # add required username field
        username_field = FIXValidator.StringField(553, optional=False)
        username_field.known_as('Username')
        m.add_field_parser(username_field)

        with self.assertRaisesRegex(FIXValidator.RejectError, 'Unexpected tag 554 found in Logon message') as cm:
            data = v.validate(msg)
        self.assertEqual(cm.exception.sessionRejectReason, 2)

        # add required password field
        password_field = FIXValidator.StringField(554, optional=False)
        password_field.known_as('Password')
        m.add_field_parser(password_field)

        resetseqno_field = FIXValidator.CharField(141, optional=True, values=['Y', 'N'])
        ...
        with self.assertRaisesRegex(FIXValidator.BusinessRejectError, 'Incorrect value 141=Z.*expected Y,N') as cm:
            data = v.validate(msg)

        # Reject message missing a required field
        msg = message.from_delim('8=FIX.4.2|9=59|35=A|34=1|49=ARCA|553=chris|52=20150916-04:14:05.306|56=TW|10=158|')
        with self.assertRaisesRegex(FIXValidator.RejectError, 'Required tag 554.*Logon') as cm:
            data = v.validate(msg)
        self.assertEqual(cm.exception.sessionRejectReason, 1)

    def test_dupe_message_parser(self):
        m = FIXValidator.Message('A', 'Logon')
        b = FIXValidator.Message('A', 'Logout')
    ...
    def test_msg_ordered(self):
        # Order: 553, 141?, 554
        m = FIXValidator.Message('A', 'Logon')
        m.add_field_parser(FIXValidator.StringField(553, optional=False).known_as('Username'))
        m.add_field_parser(FIXValidator.CharField(141, optional=True, values=['Y', 'N']).known_as('ResetSeqNumFlag'))
        m.add_field_parser(FIXValidator.StringField(554, optional=False).known_as('Password'))
        m.set_ordered(True)
        ...
        msg = message.from_delim('8=FIX.4.2|9=76|35=A|34=1|49=ARCA|553=chris|554=secret|141=Y|52='
                                    '20150916-04:14:05.306|56=TW|10=044|')
        with self.assertRaisesRegex(FIXValidator.RejectError, 'Optional field 141 .* out of order.*553, 141.*'):
            data = v.validate(msg)
    ...
    def test_repeating_group_optional_fields(self):
        ...
        msg = message.from_delim(
                ...
                '269=1^270=3721.8^271=200^10=083^', delim='^')

        m = FIXValidator.Message('W', 'MarketDataSnapshot')
        m.add_field_parser(FIXValidator.StringField(262).known_as('req_id'))
        m.add_field_parser(FIXValidator.StringField(55).known_as('symbol'))

        s = FIXValidator.RepeatingGroup(m, 'MDEntries')
        s.add_field_parser(FIXValidator.CharField(269, optional=False).known_as('MDEntryType'))
        s.add_field_parser(FIXValidator.FloatField(270, optional=True).known_as('MDEntryPx'))
        s.add_field_parser(FIXValidator.FloatField(271, optional=True).known_as('MDEntrySz'))
        s.add_field_parser(FIXValidator.StringField(278, optional=True).known_as('MDEntryID'))
        no_syms = FIXValidator.RepeatingGroupLengthField(268, s).known_as('NoEntries')
        ...
        self.assertEqual(data, {'req_id': '15e4c9106c', 'symbol': 'XBTUSD', 'md_entries': [
    ...
    def test_repeating_group_embed(self):
        ...
        m = FIXValidator.Message('S', 'MarketDataSubscribe')
        m.add_field_parser(FIXValidator.StringField(553, optional=False).known_as('Before'))

        s = FIXValidator.RepeatingGroup(m, 'symbols')
        s.add_field_parser(FIXValidator.StringField(55, optional=False).known_as('Symbol'))
        no_syms = FIXValidator.RepeatingGroupLengthField(57, s).known_as('NoSymbols')

        m.add_field_parser(no_syms)
        m.add_field_parser(FIXValidator.StringField(554, optional=False).known_as('After'))
        m.set_ordered(True)
        ...
        s.add_field_parser(FIXValidator.IntField(58, optional=False).known_as('Type'))
    ...
    def test_repeating_group_bare(self):
        ...
        m = FIXValidator.Message('U', 'MarketDataUnsubscribe')
        s = FIXValidator.RepeatingGroup(m, 'Symbols')
        s.add_field_parser(FIXValidator.StringField(55, optional=False).known_as('Symbol'))
        no_syms = FIXValidator.RepeatingGroupLengthField(57, s).known_as('NoSymbols')
        m.add_field_parser(no_syms)
        ...
        # check empty group
        msg = message.from_delim('8=FIX.4.2|9=54|35=U|34=1|49=ARCA|57=0|52=20150916-04:14:05.306|56=TW|10=147|')
        data = v.validate(msg)
        ...
        with self.assertRaisesRegex(
                ...
                re.escape("Repeating Group 'Symbols' started by 57=2 (NoSymbols) had 3 repeats, not 2, in "
                          "MarketDataUnsubscribe[U] message")) as cm:
            data = v.validate(msg)
            print(data)
    ...
    def test_nestedclass_validator(self):
        bfv = FIXValidator.FIX44Validator()
        bfv.build()
        # bfv.print()
