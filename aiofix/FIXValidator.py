...
"""
Above are the minimal hand-wired validation mechanism. The classes below lay out
production validators using nested python classes
"""


class MetaclassFIXValidator(BaseFIXValidator):
    def build(self):
        for d in dir(self):
            o = getattr(self, d)
            if inspect.isclass(o) and not d == '__class__':
                ...
                for value in [x for x in dir(o) if not x.startswith('_')]:
                    ...
                    if value == 'msgtype':
                        ...
                        raise RuntimeError("Unknown field/property {} {}".format(value, type(o2)))
                ...
                for k, v in rpts:
                    ...
                    if rg_fields:
                        raise RuntimeError("Field {} not in ordering".format(rg_fields.keys()))

                    no_syms = RepeatingGroupLengthField(counter[0], s).known_as(counter[1])
                ...
                self.add_message_parser(m)
        self._built = True


class FIX44BaseValidator(MetaclassFIXValidator):
    class Heartbeat():
        msgtype = '0'
...
class FIX44Validator(FIX44BaseValidator):
    class LogonMessage():
        msgtype = 'A'
        ResetSeqNum = CharField(141, values=['Y', 'N'], optional=True)
        Username = StringField(553, optional=True)  # optional as validator used in client too
